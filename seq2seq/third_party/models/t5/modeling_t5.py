...
from seq2seq.inplace_layernorm import InplaceT5LayerNorm
...
class LadderAdapterModule(nn.Module):
    def __init__(self, config, adapter_config):
        super().__init__()
        self.adapter_layer = AdapterController(adapter_config)
        self.layer_norm = T5LayerNorm(config.d_model, eps=config.layer_norm_epsilon)
        self.dropout = nn.Dropout(config.dropout_rate)

    def forward(self, side_hidden_state, task):
        forward_hidden = self.layer_norm(side_hidden_state)
        forward_hidden = self.adapter_layer(forward_hidden, task) 
        forward_hidden = self.dropout(forward_hidden) + side_hidden_state

        return forward_hidden

    def init_weights(self):
        for k, module in self.adapter_layer.adapters.items():
            self.adapter_layer.adapters[k].up_sampler.weight.data.normal_(0, 1e-10)
            self.adapter_layer.adapters[k].down_sampler.weight.data.normal_(0, 1e-10)
...
class T5DenseReluDense(nn.Module):
    def __init__(self, config, adapter_config=None, approx_config=None):
        super().__init__()
        self.bitfit = adapter_config.bitfit if adapter_config is not None else False 
        ...
        if approx_config is not None and approx_config.quant_relu:
            self.act = QReLU()
        else:
            self.act = nn.ReLU()

        if approx_config is not None and approx_config.quant_dropout:
            self.dropout = QDropout(config.dropout_rate)
        else:
            self.dropout = nn.Dropout(config.dropout_rate)

        if approx_config is not None:

            if approx_config.wi_0_sampling or approx_config.wi_1_sampling:
                self.wi = ApproxLinear(config.d_model, config.d_ff, bias=False if not self.bitfit else True,
                                      config=approx_config)

            if approx_config.wo_sampling:
                self.wo = ApproxLinear(config.d_ff, config.d_model, bias=False if not self.bitfit else True,
                                      config=approx_config)

            self.use_approx = approx_config.wi_0_sampling or approx_config.wi_1_sampling or approx_config.wi_1_sampling
...
class T5DenseGatedGeluDense(nn.Module):
    def __init__(self, config, adapter_config=None, approx_config=None):
        ...
        if approx_config is not None:
            ...
            if approx_config.wi_0_sampling:
                self.wi_0 = ApproxLinear(config.d_model, config.d_ff, bias=False if not self.bitfit else True,
                                      config=approx_config)

            if approx_config.wi_1_sampling:
                self.wi_1 = ApproxLinear(config.d_model, config.d_ff, bias=False if not self.bitfit else True,
                                      config=approx_config)
...
class T5LayerFF(nn.Module):
    def __init__(self, config, adapter_config=None, approx_config=None):
        ...
        if config.feed_forward_proj == "relu":
            self.DenseReluDense = T5DenseReluDense(config, adapter_config=adapter_config, approx_config=approx_config)
        elif config.feed_forward_proj == "gated-gelu": # TODO Inplace version
            self.DenseReluDense = T5DenseGatedGeluDense(config, adapter_config=adapter_config, approx_config=approx_config)
        else:
            raise ValueError(
        ...
        if approx_config is not None and approx_config.inplace_layernorm:
            self.layer_norm = InplaceT5LayerNorm(config.d_model, eps=config.layer_norm_epsilon, adapter_config=adapter_config)
        else:
...
class T5Attention(nn.Module):
    def __init__(self, config: T5Config, has_relative_attention_bias=False, adapter_config=None, lora_config=None, approx_config=None):
        super().__init__()
        self.bitfit = adapter_config.bitfit if adapter_config is not None else False 
        ...
        self.score_matmul_op = torch.matmul
        self.attout_matmul_op = torch.matmul
        self.softmax_op = torch.softmax
        ...
        if lora_config is not None:
            self.use_lora = True
            self.q = LoRALinearController(self.d_model, self.inner_dim, config=lora_config, bias=False if not self.bitfit else True)
            self.v = LoRALinearController(self.d_model, self.inner_dim, config=lora_config, bias=False if not self.bitfit else True)
        ...
        if approx_config is not None:
            ...
            if approx_config.q_sampling:
                self.q = ApproxLinear(self.d_model, self.inner_dim, bias=False if not self.bitfit else True,
                                      config=approx_config)

            if approx_config.v_sampling:
                self.v = ApproxLinear(self.d_model, self.inner_dim, bias=False if not self.bitfit else True,
                                  config=approx_config)

            # has some practical problem
            if approx_config.k_sampling:
                self.k = ApproxLinear(self.d_model, self.inner_dim, bias=False if not self.bitfit else True,
                                  config=approx_config)

            ## accuracy drop is too large
            if approx_config.o_sampling:
                self.o = ApproxLinear(self.inner_dim, self.d_model, bias=False if not self.bitfit else True,
                                  config=approx_config)

            if approx_config.score_sampling:
                self.score_matmul_op = Approxmatmul_4D(config=approx_config, batch_dim_use_same_indices=False)
    ...
    def forward(
        ...
        if self.use_lora:
            self.q.forward = partial(self.q.forward, task=task)
            self.v.forward = partial(self.v.forward, task=task)
        ...
        #         scores
        #     )
        attn_weights = self.softmax_op(scores.float(), dim=-1).type_as(
            scores
        )  # (batch_size, n_heads, seq_length, key_length)
        attn_weights = self.dropout_op(
        ...
        attn_output = unshape(self.attout_matmul_op(attn_weights, value_states))  # (batch_size, seq_length, dim)
...
class T5LayerSelfAttention(nn.Module):
    def __init__(self, config, has_relative_attention_bias=False, adapter_config=None, lora_config=None, approx_config=None):
        super().__init__()
        self.SelfAttention = T5Attention(config, has_relative_attention_bias=has_relative_attention_bias, adapter_config=adapter_config, 
                                         lora_config=lora_config, approx_config=approx_config)
        # self.layer_norm = T5LayerNorm(config.d_model, eps=config.layer_norm_epsilon, adapter_config=adapter_config)
...
class T5LayerCrossAttention(nn.Module):
    def __init__(self, config, adapter_config=None, lora_config=None, approx_config=None):
        super().__init__()
...
class T5Block(nn.Module):
    def __init__(self, config, has_relative_attention_bias=False, adapter_config=None, lora_config=None, approx_config=None):
        ...
        self.layer.append(T5LayerSelfAttention(config,has_relative_attention_bias=has_relative_attention_bias, adapter_config=adapter_config, lora_config=lora_config, approx_config=approx_config))
        if self.is_decoder:
            self.layer.append(T5LayerCrossAttention(config, adapter_config=adapter_config, lora_config=lora_config, approx_config=approx_config))

        self.layer.append(T5LayerFF(config, adapter_config=adapter_config, approx_config=approx_config))

        self.use_gate = config.use_gate

        if config.train_side_ladder:
            adapter_config.reduction_factor = adapter_config.task_reduction_factor
            self.side_ladder_module = nn.ModuleList()
            self.side_gate_params = nn.ParameterList()

            self.side_ladder_module.append(LadderAdapterModule(config, adapter_config))

            # self.side_gate_params.append(nn.Parameter(torch.ones(1) * 0.0))
            ...
            if self.is_decoder:
                ...
                side_config.d_ff = side_config.d_ff // adapter_config.task_reduction_factor
                side_config.d_kv = side_config.d_kv // adapter_config.task_reduction_factor
                side_config.d_model = side_config.d_model // adapter_config.task_reduction_factor

                self.side_upsample = nn.Linear(side_config.d_model, config.d_model)
                self.side_downsample = nn.Linear(config.d_model, side_config.d_model)
                self.side_ladder_module.append(T5LayerCrossAttention(side_config, adapter_config=adapter_config))
                # self.side_gate_params.append(nn.Parameter(torch.ones(1) * 0.0))

            self.side_ladder_module.append(LadderAdapterModule(config, adapter_config))

            if self.use_gate == "learnable":
                self.side_gate_params.append(nn.Parameter(torch.ones(1) * config.gate_alpha))
                self.gate_T = config.gate_T
            elif "schedule" in self.use_gate or self.use_gate == "one":
                self.register_buffer('side_gate_last', torch.ones(1))
            ...
            self.side_ladder_module = None

    def forward(
        self,
        hidden_states,
        side_hidden_state=None,
        encoder_side_hidden_states=None,
        attention_mask=None,
        position_bias=None,
        ...
        if side_past_key_value is not None:
            assert self.is_decoder, "Only decoder can use `side_past_key_values`"
            side_cross_attn_past_key_value = side_past_key_value[:2]
        else:
            side_cross_attn_past_key_value = None
        ...
        if self.side_ladder_module is not None:
            # gate = torch.sigmoid(self.side_gate_params[0] * 10)
            # side_hidden_state = gate * side_hidden_state + (1 - gate) * hidden_states

            side_hidden_state = side_hidden_state# + hidden_states
            side_hidden_state = self.side_ladder_module[0](side_hidden_state, task)

            if side_hidden_state.dtype == torch.float16 and torch.isinf(side_hidden_state).any():
                clamp_value = torch.finfo(side_hidden_state.dtype).max - 1000
                side_hidden_state = torch.clamp(side_hidden_state, min=-clamp_value, max=clamp_value)
        ...
        if do_cross_attention:
            ...
            if self.side_ladder_module is not None:
                # gate = torch.sigmoid(self.side_gate_params[1] * 10)
                # side_hidden_state = gate * side_hidden_state + (1 - gate) * hidden_states

                side_hidden_state = side_hidden_state# + hidden_states

                side_hidden_state = self.side_downsample(side_hidden_state)

                encoder_side_hidden_states = self.side_downsample(encoder_side_hidden_states)

                side_hidden_state, side_present_key_value_state = self.side_ladder_module[1](
                    side_hidden_state,
                    key_value_states=encoder_side_hidden_states,
                    attention_mask=encoder_attention_mask,
                    position_bias=encoder_decoder_position_bias,
                    layer_head_mask=cross_attn_layer_head_mask,
                ...
                side_hidden_state = self.side_upsample(side_hidden_state)
                # side_hidden_state = side_hidden_state + encoder_side_hidden_states.mean(dim=1, keepdim=True)
                # side_hidden_state = self.side_ladder_module[1](side_hidden_state, task) + hidden_states
                if side_hidden_state.dtype == torch.float16 and torch.isinf(side_hidden_state).any():
                    clamp_value = torch.finfo(side_hidden_state.dtype).max - 1000
                    side_hidden_state = torch.clamp(side_hidden_state, min=-clamp_value, max=clamp_value)
        ...
        if self.side_ladder_module is not None:
            ...
            if self.use_gate == "learnable":
                side_gate_param = self.side_gate_params[-1]
                gate = torch.sigmoid(side_gate_param / self.gate_T)
                side_hidden_state = gate * side_hidden_state + (1 - gate) * hidden_states # add the information from the backbone network
            elif "schedule" in self.use_gate or self.use_gate == "one":
                gate = getattr(self, f"side_gate_last")
                side_hidden_state = gate * side_hidden_state + (1 - gate) * hidden_states # add the information from the backbone network
            else:
                side_hidden_state = side_hidden_state + hidden_states

            side_hidden_state = self.side_ladder_module[-1](side_hidden_state, task)
        ...
        outputs = (hidden_states, side_hidden_state)

        if use_cache:
            outputs = outputs + (present_key_value_state, side_present_key_value_state) + attention_outputs
        else:
            outputs = outputs + attention_outputs

        return outputs  # hidden-states, side_hidden_state, present_key_value_states, (self-attention weights), (self-attention position bias), (cross-attention weights), (cross-attention position bias)
...
class T5Stack(T5PreTrainedModel):
    def __init__(self, config, embed_tokens=None, adapter_config=None, prefix_emb=None, lora_config=None, approx_config=None):
        ...
        self.block = nn.ModuleList(
            [T5Block(self.per_layer_config(config, i, self.adapter_config, self.is_decoder),
                     ...
                     lora_config=self.per_layer_lora_config(lora_config, i, self.adapter_config, self.is_decoder),
        ...
        if approx_config is not None and approx_config.inplace_layernorm:
            self.final_layer_norm = InplaceT5LayerNorm(config.d_model, eps=config.layer_norm_epsilon, adapter_config=adapter_config)
        else:
        ...
        self.train_side_ladder = config.train_side_ladder
        if config.train_side_ladder:
            # if self.is_decoder:
                # for i in range(len(self.block)):
                    # self.block[i].side_upsample = self.block[0].side_upsample
                    # self.block[i].side_downsample = self.block[0].side_downsample
            # self.final_side_layer_norm = T5LayerNorm(config.d_model, eps=config.layer_norm_epsilon)
            if approx_config is not None and approx_config.inplace_layernorm:
                self.final_side_layer_norm = T5LayerNorm(config.d_model, eps=config.layer_norm_epsilon, adapter_config=adapter_config)
            else:
                self.final_side_layer_norm = InplaceT5LayerNorm(config.d_model, eps=config.layer_norm_epsilon, adapter_config=adapter_config)
        else:
            self.final_side_layer_norm = None
    ...
    def per_layer_lora_config(self, lora_config, layer_id, adapter_config, is_decoder):
        ...
        valid_lora_layer_id = is_valid_layer(layer_id, adapter_config, is_decoder)

        lora_config = copy.deepcopy(lora_config)

        return lora_config if valid_lora_layer_id else None
    ...
    def per_layer_config(self, config, layer_id, adapter_config, is_decoder):
        ...
        # Also control the lora config
        if not valid_task_adapter_layer_id:
    ...
    def parallelize(self, device_map=None):
        ...
        if self.final_side_layer_norm is not None:
    ...
    def deparallelize(self):
        ...
        self.final_side_layer_norm = self.final_side_layer_norm.to("cpu")
    ...
    def forward(
        ...
        if side_past_key_values is None:
            side_past_key_values = [None] * len(self.block)
        ...
        side_present_key_value_states = () if use_cache else None
        ...
        if self.train_side_ladder:
            side_hidden_state = hidden_states
        else:
            side_hidden_state = None
        ...
        for i, (layer_module, past_key_value) in enumerate(zip(self.block, past_key_values)):
            ...
            if self.model_parallel:
                ...
                if encoder_side_hidden_states is not None:
                    encoder_side_hidden_states = encoder_side_hidden_states.to(hidden_states.device)
            ...
            if output_hidden_states:
                ...
                all_side_hidden_states = all_side_hidden_states + (side_hidden_state,)
            ...
            if getattr(self.config, "gradient_checkpointing", False) and self.training:
                ...
                layer_outputs = layer_module(
                    hidden_states,
                    side_hidden_state=side_hidden_state,
                    encoder_side_hidden_states=encoder_side_hidden_states,
                    ...
                    past_key_value=past_key_value,
                    side_past_key_value=side_past_key_value,
                    use_cache=use_cache,
                    output_attentions=output_attentions,
                    task=task
                )
            ...
            hidden_states, side_hidden_state, present_key_value_state, side_present_key_value_state = layer_outputs[:4]

            # We share the position biases between the layers - the first layer store them
            # layer_outputs = hidden-states, key-value-states (self-attention weights),
            # (self-attention position bias), (cross-attention weights), (cross-attention position bias)
            position_bias = layer_outputs[4]
            if self.is_decoder and encoder_hidden_states is not None:
                encoder_decoder_position_bias = layer_outputs[6 if output_attentions else 5]
            # append next layer key value states
            if use_cache:
                present_key_value_states = present_key_value_states + (present_key_value_state,)
                side_present_key_value_states = side_present_key_value_states + (side_present_key_value_state,)
        ...
        if self.final_side_layer_norm is not None:
            side_hidden_state = self.final_side_layer_norm(side_hidden_state)
            side_hidden_state = self.dropout(side_hidden_state)
        ...
        if not return_dict:
            return tuple(
                ...
                for v in [
                    ...
                    side_present_key_value_states,
                ]
                if v is not None
            )
        return SideBaseModelOutputWithPastAndCrossAttentions(
            last_hidden_state=hidden_states,
            last_side_hidden_state=side_hidden_state,
            side_hidden_states=all_side_hidden_states,
            past_key_values=present_key_value_states,
            side_past_key_values=side_present_key_value_states,
...
class T5Model(T5PreTrainedModel):
    ...
    def forward(
        ...
        side_hidden_state = encoder_outputs.last_side_hidden_state
        if self.model_parallel:
        ...
        if self.model_parallel:
            ...
            if side_hidden_state is not None:
                side_hidden_state = side_hidden_state.to(self.decoder.first_device)
        ...
        decoder_outputs = self.decoder(
            ...
            encoder_side_hidden_states=side_hidden_state,
        ...
        return SideSeq2SeqModelOutput(
            ...
            decoder_last_side_hidden_state=decoder_outputs.last_side_hidden_state,
            decoder_side_hidden_states=decoder_outputs.side_hidden_states,
            cross_attentions=decoder_outputs.cross_attentions,
            encoder_last_hidden_state=encoder_outputs.last_hidden_state,
            encoder_hidden_states=encoder_outputs.hidden_states,
            encoder_attentions=encoder_outputs.attentions,
            encoder_last_side_hidden_state=encoder_outputs.last_side_hidden_state,
            encoder_side_hidden_states=encoder_outputs.side_hidden_states,
        )
...
class T5ForConditionalGeneration(T5PreTrainedModel):
    ...
    def __init__(self, config, adapter_config=None, lora_config=None, approx_config=None):
        ...
        encoder_config.use_cache = False
        encoder_config.is_encoder_decoder = False
        self.encoder = T5Stack(encoder_config, self.shared, adapter_config=adapter_config, prefix_emb=self.prefix_shared, 
                               lora_config=lora_config, approx_config=approx_config)
    ...
    def forward(
        ...
        if decoder_outputs.last_side_hidden_state is not None:
