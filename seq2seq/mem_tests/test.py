...
def test_attention_mem():
    ...
    approx_config.level = 0
    approx_config.lfc_block = 8
    approx_config.hfc_bit = 4
    approx_config.tasks = ["sst2"]
    ...
    for dtype in ['float32']:
        ...
        def test_implementation(func):
            ...
            print("After Forward Usage: %.2f MB" % (after / 2 ** 20))

            data[0].sum().backward()
            final = get_memory_usage()

            print("After Backward Usage: %.2f MB" % (final / 2 ** 20))
